...
mod blame_cache_to_hunks {
    use super::*;
    use crate::file::{checkpoint::update_checkpoint_blames_with_changes, BlameEntry, Change, UnblamedHunk};
    use crate::BlameCheckpoint;

    fn single_blame_entry() -> BlameCheckpoint {
        BlameCheckpoint {
            entries: vec![BlameEntry::new(0..5, 0..5, zero_sha())],
            checkpoint_commit_id: zero_sha(),
        }
    }

    fn multiple_blame_entries() -> BlameCheckpoint {
        BlameCheckpoint {
            entries: vec![
    ...
    #[test]
    fn no_changes() {
        let checkpoint = single_blame_entry();
        let changes = vec![Change::Unchanged(0..5)];

        let expected_blame = single_blame_entry().entries;
        let (updated_blame_entries, new_unblamed_hunks) =
            update_checkpoint_blames_with_changes(checkpoint, changes, one_sha());
    ...
    fn add_or_replace_full_blame_delete() {
        ...
        let (updated_blame_entries, new_unblamed_hunks) =
            update_checkpoint_blames_with_changes(checkpoint, changes, two_sha());
        assert_eq!(updated_blame_entries, expected_blame);
    ...
    fn multiple_blames_no_change() {
        let checkpoint = multiple_blame_entries();
        let changes = vec![Change::Unchanged(0..10)];
        let expected_blame = multiple_blame_entries().entries;
        let (updated_blame_entries, new_unblamed_hunks) =
            update_checkpoint_blames_with_changes(checkpoint, changes, one_sha());
        assert_eq!(updated_blame_entries, expected_blame);
        assert!(new_unblamed_hunks.is_empty());
